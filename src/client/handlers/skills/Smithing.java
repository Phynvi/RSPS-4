...
import server.resources.Messages;
...
public class Smithing {
	...
	public void smeltBar(String barType){
		...
		c.getSkillHandler().startSkillTimerForSkill(SMELTING_ANIMATION_DELAY, 5);
	...
	private static final int SMELTING_ANIMATION_DELAY = 3;
	
	public void SmithItem(){
		
		if(c.getInventoryHandler().hasItemOfAtLeastAmount(_removeBar, _removeBarAmount)){

			c.startAnimation(898);
			
			for(int i = 0; i < _removeBarAmount; i++)
				c.getInventoryHandler().deleteItem(_removeBar);
			
			c.getInventoryHandler().addItem(_deliverItem, _deliverItemAmount);
			
			_totalRemainingItemsToSmith -= 1;
			
			c.getClientMethodHandler().addSkillXP(smithXP(_removeBar, _removeBarAmount), c.playerSmithing);
	...
	private void LoopSmithItem(){
		if(_totalRemainingItemsToSmith > 0){
			c.getSkillHandler().setSkillType(10);
			c.getSkillHandler().setSkillTimer(5);
		}
	}
	
	private int _removeBar;
	private int _removeBarAmount;
	private int _deliverItem;
	private int _deliverItemAmount;
	private int _totalRemainingItemsToSmith;
	
	public boolean removeBarAndStartProcessSmithItem(int interfaceID, int removeID, int removeSlot, int maxAmountOfTimes){
		//Smith Column 1,2,3,4
		if (interfaceID == 1119 || interfaceID == 1120 || interfaceID == 1121 || interfaceID == 1122 || interfaceID == 1123){
			try { 
				int amountOfTimes = c.getInventoryHandler().amountOfItemInInventory(getRequiredBarType(removeID));
				...
				if(amountOfTimes > 0) {
					if(c.getInventoryHandler().playerHasItem(2347)) {
						...
						if(c.playerLevel[c.playerSmithing] >= lvlRequired) {
							...
							_deliverItem = removeID;
							_deliverItemAmount = 1;
							if(interfaceID == 1123){
								if(removeSlot == 0)
									_deliverItemAmount = 10;
								if(removeSlot == 1)
									_deliverItemAmount = 15;
								if(removeSlot == 2)
									_deliverItemAmount = 5;
							}
							
							_removeBar = getRequiredBarType(removeID);
							_removeBarAmount = barsNeeded(removeSlot, interfaceID);
							_totalRemainingItemsToSmith = amountOfTimes;
					...
					c.sendMessage(Messages.NotEnoughSmithingBars);
				}
			} 
			catch(Exception e) { 
	...
	/*SMITHING*/
	public int getRequiredBarType(int removeID) {
	...
	public int smithXP(int barType, int barAmount) {
		if (barType == 2357) return barAmount*25*c.rate;
