...
public class FrameMethods {
	 ...
	 * The gfx will be visible to all players.
	 */
	public static void createAllGfxWithDelay(int delay, int gfxId, int x, int y, int height){
		for(Player p : server.playerHandler.players){
			if(p != null && p.distanceToPoint(x, y) <= 60){
				((client)p).getFrameMethodHandler().gfxWithDelay(gfxId, x, y, delay, height);
			}
		}
	}
