...
import client.handlers.FoodHandler;
import client.handlers.FrameMethods;
import client.handlers.InventoryHandler;
import client.handlers.Item;
import client.handlers.ItemUse;
import client.handlers.MenuHandler;
...
import client.handlers.skills.RangeDataHandler;
import client.handlers.skills.Runecrafting;
import client.handlers.skills.Slayer;
import client.handlers.skills.Smithing;
import client.handlers.skills.Woodcutting;

import java.util.Calendar;
import java.util.GregorianCalendar;
import java.security.*;

import server.handlers.NPC.NPCClickHandler;
...
import server.resources.ISAACCipher;
...
import server.tests.PlayerTests;
...
public class client extends Player implements Runnable {
	...
	public int travel2_X1 = -1;
	public int travel2_Y1 = -1;
	public int travel2_X2 = -1;
	public int travel2_Y2 = -1;
	public int travelHeight = 0;
...
playerName.trim();*/
    ...
    System.out.println("Player pass hash = "+hash);
			...
			if(PlayerHandler.getPlayerCount() >= PlayerHandler.maxPlayers) {
				returnCode = 7;
				savefile = false;
				disconnected = true;
	...
	public void initialize()
	{
		...
		if(playerName.equalsIgnoreCase(AllTests.testPlayerName)){
	...
	public void checkSpecialTimers(){
		...
		if (SpecTimer == 1 && (this.getEnemy() != null)){
			...
			if (playerEquipment[playerWeapon] == 4153){ // g maul
				this.getEnemy().inflictMeleeDamage(this.getCombatHandler().getMaxMeleeHit(), this.PlayerAsEnemy,0);
				FrameMethods.createAllGfxWithDelayInMS(1, 337, this.getEnemy().getX(), this.getEnemy().getY());
				startAnimation(1667);
			}
			...
			if (playerEquipment[playerWeapon] == 5698 || 
				...
				this.getEnemy().inflictMeleeDamage(this.getCombatHandler().getMaxMeleeHit() + misc.random(playerLevel[playerAttack]/11), this.PlayerAsEnemy,4);
			}

			if(playerEquipment[playerWeapon] == 861){ //magic shortbow
				startAnimation(426);
				this.getEnemy().inflictRangeDamage(this.getCombatHandler().getMaxRangedHit() + misc.random(playerLevel[playerRanged]/25), this.PlayerAsEnemy,0);
			}
			
			if(playerEquipment[playerWeapon] == Item.DARKBOW){ 
				startAnimation(426);
				int maxHit = this.getCombatHandler().getMaxRangedHit();
				this.getEnemy().inflictRangeDamage(maxHit + (int)((double)maxHit*0.3), this.PlayerAsEnemy,0);
			}
		...
		if (DClawsHit1 == true && (this.getEnemy() != null) && DClawsTimer == 8){
			if (DClawsDmg > 0){ //if first hit is greater than 0
				...
				this.getEnemy().inflictMeleeDamage(DClawsHit2, this.PlayerAsEnemy,0);
		...
		if (this.getEnemy() != null && DClawsTimer == 7){
			this.getEnemy().inflictMeleeDamage(DClawsHit3, this.PlayerAsEnemy,0);
		}
		
		if (this.getEnemy() != null && DClawsTimer == 6){
			this.getEnemy().inflictMeleeDamage(DClawsHit4, this.PlayerAsEnemy,0);
		}
	}
	...
	public boolean process() { 	// is being called regularily every 500ms	
		...
		if (IsDead == true && NewHP <= 0 && deadAnimTimer == -1){ 
			...
			if(getSkillHandler().getPrayerHandler().Retribution){				
				...
				Combat.attackEnemiesWithin(-1, -1, false, this.PlayerAsEnemy, 3, maxDamage, this.PlayerAsEnemy, 2, this.PlayerAsEnemy, true, true);
	...
	public void parseIncomingPackets(){
		...
		case 202:			// idle logout packet - ignore for now
			if(this.playerRights < 1) this.disconnectPlayerAndSave("Idle");
			break;
...
//			break;
		...
		case 249: //Magic on Players
			...
			Enemy e = server.playerHandler.players[playerIndex].PlayerAsEnemy;
			if(Combat.CanEnemyAttackTarget(this.PlayerAsEnemy, e)){
		...
		case 131: //Magic on NPCs
			...
			e = server.npcHandler.npcs[npcIndex].getAsEnemy();
		...
		case 145:	//remove item (opposite for wearing) - bank 1 item - value of item
			...
			else getSkillHandler().getSmithingHandler().removeBarAndStartProcessSmithItem(interfaceID, removeID, removeSlot, 1);
		...
		case 117:	//bank 5 items - sell 1 item
			...
			else getSkillHandler().getSmithingHandler().removeBarAndStartProcessSmithItem(interfaceID, removeID, removeSlot, 5);
		...
		case 43:	//bank 10 items - sell 5 items
			...
			else getSkillHandler().getSmithingHandler().removeBarAndStartProcessSmithItem(interfaceID, removeID, removeSlot, 10);
		...
		case 129:	//bank all items - sell 10 items
			...
			if (interfaceID == 5064) { //remove from bag to bank
				if (Item.itemStackable[removeID] == true) {
					...
					getInventoryHandler().bankItem(playerItems[removeSlot] , removeSlot, getInventoryHandler().itemAmount(playerItems[removeSlot]-1));
				}
			} 
			else if (interfaceID == 5382) { //remove from bank
