...
public int getBonus(){
	...
	private int getArrowGFX(){
		...
		c.debug("player ammo not found in bowHandler, ID : "+curAmmo);
		return -1;
	}
	...
	public void arrowProjectile(int npcIndex){
		...
		c.createProjectile(c.absY, c.absX, offsetX, offsetY, 50, 100, getArrowGFX(), 43, 31, npcIndex+1);
	}
	...
	public void arrowProjectilePlayer(int playerIndex){
		...
		int arrowGFX = getArrowGFX();
		//c.ProjectileSpellPlayer(arrowGFX, arrowGFX, arrowGFX, c.absY, c.absX, offsetY, offsetX, c.AttackingOn+1, Y3, X3);
		c.createProjectile(c.absY, c.absX, offsetX, offsetY, 50, 80, getArrowGFX(), 43, 31, c.AttackingOn+1);
	}
